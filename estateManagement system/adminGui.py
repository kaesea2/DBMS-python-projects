...
class AdminGui(Tk):
    def __init__(self):
        super().__init__()
        # containers
        self.Estate = Estate.Estates
        self.Thoroughfare = Thoroughfare.Thoroughfares
        self.Property = Property.Properties
        self.Household = Household.Households
        self.Receipt = Receipt.receipts
        ...
        # menu functions
        self.estateFunctions = ['CREATE ESTATE', 'DELETE ESTATE', 'VIEW ESTATE', 'UPDATE ESTATE']
        self.thoroughfareFunctions = ['CREATE THOROUGHFARE', 'DELETE THOROUGHFARE', 'VIEW THOROUGHFARE', 'UPDATE THOROUGHFARE']
        ...
        self.receiptFunctions = ['CREATE RECEIPT', 'DELETE RECEIPT', 'VIEW RECEIPT', 'UPDATE RECEIPT']
        ...
        self.exit_button = Button(self.footerFrame, text='EXIT', padx=20)
        self.manager_button = Button(self.footerFrame, text='MANAGER', padx=20)

        self.exit_button.grid(row=0, column=0, sticky=N)
        self.manager_button.grid(row=0, column=1, sticky=N)

        self.exit_button.configure(command=self.destroy)
        self.manager_button.configure(command=self.managerClicked)

    def managerClicked(self):
        self.withdraw()
        self.manager_gui = ManagerGui()
        self.manager_gui.mainloop()
    ...
    def estate_traceBack(self, *args):
        ...
        elif self.estateVar.get() == 'UPDATE ESTATE':
            self.labeltest.grid(row=0, column=0, columnspan=2)
            self.updateEstateClicked()
    ...
    def receipt_traceBack(self, *args):
        ...
        elif self.receiptVar.get() == 'DELETE RECEIPT':
            self.labeltest.grid(row=0, column=0, columnspan=2)
            self.deleteReceiptClicked()
        elif self.receiptVar.get() == 'VIEW RECEIPT':
            self.viewReceiptClicked()
        elif self.receiptVar.get() == 'UPDATE RECEIPT':
            self.labeltest.grid(row=0, column=0, columnspan=2)
            self.updateReceiptClicked()
...
# estate functions
    ...
    def create_estate(self):
        ...
        if estate not in self.Estate:
            ...
            messagebox.showinfo(title="SUCCESS!!!", message="estate created successfully!")
        else:
            messagebox.showerror(title="ERROR!!!", message="create estate failed!")
    ...
    def viewEstateClicked(self):
        ...
        if len(self.Estate) != 0:
            for estate in self.Estate:
                self.view_estate_list.insert(0, estate)
        else:
            messagebox.showerror(title="ERROR!!!", message="no estate available!")

    def updateEstateClicked(self):
        # list components
        self.update_enter_label = Label(self.showFrame)
        self.estate_input = Entry(self.showFrame)
        self.update_enter_label2 = Label(self.showFrame)
        self.new_estate_input = Entry(self.showFrame)
        self.update_button = Button(self.showFrame)
        # add component to window
        self.update_enter_label.grid(row=1, column=0)
        self.estate_input.grid(row=1, column=1)
        self.update_enter_label2.grid(row=2, column=0)
        self.new_estate_input.grid(row=2, column=1)
        self.update_button.grid(row=3, column=0, columnspan=2)
        # style the components
        self.update_enter_label.configure(text='estate name', bg='#fff')
        self.estate_input.configure(width=15)
        self.update_enter_label2.configure(text='new name', bg='#fff')
        self.new_estate_input.configure(width=15)
        self.update_button.configure(text='UPDATE', padx=65)
        # assign event
        self.update_button.configure(command=self.update_estate)

    def update_estate(self):
        estate = Estate(self.estate_input.get()).show()
        newEstate = Estate(self.new_estate_input.get()).show()
        if estate in self.Estate:
            for i, item in enumerate(self.Estate):
                if item == estate:
                    self.Estate[i] = newEstate
                    messagebox.showinfo(title="SUCCESS!!!", message="estate updated successfully!")
        else:
            messagebox.showerror(title="ERROR!!!", message="estate update failed!")
        self.estate_input.delete(0, 'end')
        self.new_estate_input.delete(0, 'end')
...
# thoroughfare functions
    ...
    def update_thoroughfare(self):
        ...
        thoroughfare = Thoroughfare(thoroughfare_name, thoroughfare_type, property_position).show()
        if thoroughfare in self.Thoroughfare:
            for i, item in enumerate(self.Thoroughfare):
                if item == thoroughfare:
                    self.Thoroughfare[i] = Thoroughfare(new_thoroughfare_name, thoroughfare_type,
...
# property functions
    ...
    def update_property(self):
        ...
        property = Property(thoroughfare_name, building_name, building_number, building_type, usage_status,
                            completion_date).show()
        if property in self.Property:
            for i, item in enumerate(self.Property):
                if item == property:
                    self.Property[i] = Property(new_thoroughfare_name, new_building_name, new_building_number,
...
# household functions
    ...
    def viewHouseholdClicked(self):
        ...
        if len(self.Household) != 0:
            for household in self.Household:
    ...
    def update_household(self):
        ...
        household = Household(name, custodian, housetype).show()
        if household in self.Household:
            for i, item in enumerate(self.Household):
                if item == household:
                    self.Household[i] = Household(new_name, new_custodian, new_housetype).show()
...
# receipt functions
    ...
    def create_receipt(self):
        ...
        if receipt not in self.Receipt:
    ...
    def deleteReceiptClicked(self):
        # list components
        self.receipthousehold_label = Label(self.showFrame)
        self.receipthousehold_input = Entry(self.showFrame)
        self.receiptnumber_label = Label(self.showFrame)
        self.receiptnumber_input = Entry(self.showFrame)
        self.receiptdate_label = Label(self.showFrame)
        self.receiptdate_input = Entry(self.showFrame)
        self.delete_button = Button(self.showFrame)
        # add component to window
        self.receipthousehold_label.grid(row=1, column=0)
        self.receipthousehold_input.grid(row=1, column=1)
        self.receiptnumber_label.grid(row=2, column=0)
        self.receiptnumber_input.grid(row=2, column=1)
        self.receiptdate_label.grid(row=3, column=0)
        self.receiptdate_input.grid(row=3, column=1)
        self.delete_button.grid(row=4, column=0, columnspan=2)
        # style component
        self.receipthousehold_label.configure(text='Household name', bg='#fff')
        self.receipthousehold_input.configure(width=15)
        self.receiptnumber_label.configure(text='Receipt Number', bg='#fff')
        self.receiptnumber_input.configure(width=15)
        self.receiptdate_label.configure(text='Receipt Date', bg='#fff')
        self.receiptdate_input.configure(width=15)
        self.delete_button.configure(text='DELETE', padx=75)
        # assign event
        self.delete_button.configure(command=self.delete_receipt)

    def delete_receipt(self):
        household = self.receipthousehold_input.get()
        receipt_number = self.receiptnumber_input.get()
        receipt_date = self.receiptdate_input.get()
        receipt = Receipt(household, receipt_number, receipt_date).show()
        if receipt in self.Receipt:
            self.Receipt.remove(receipt)
            messagebox.showinfo(title="SUCCESS!!!", message="receipt deleted successfully!")
        else:
            messagebox.showerror(title="ERROR!!!", message="delete create failed!")
        self.receipthousehold_input.delete(0, 'end')
        self.receiptnumber_input.delete(0, 'end')
        self.receiptdate_input.delete(0, 'end')
    ...
    def updateReceiptClicked(self):
        ...
        self.receiptdate_label = Label(self.showFrame)
        self.receiptdate_input = Entry(self.showFrame)
        self.newreceipthousehold_label = Label(self.showFrame)
        self.newreceipthousehold_input = Entry(self.showFrame)
        self.update_button = Button(self.showFrame)
        # add component to window
        self.receipthousehold_label.grid(row=1, column=0)
        self.receipthousehold_input.grid(row=1, column=1)
        ...
        self.receiptdate_label.grid(row=3, column=0)
        self.receiptdate_input.grid(row=3, column=1)
        self.newreceipthousehold_label.grid(row=4, column=0)
        self.newreceipthousehold_input.grid(row=4, column=1)
        self.update_button.grid(row=5, column=0, columnspan=2)
        # style component
        self.receipthousehold_label.configure(text='Household name', bg='#fff')
        self.receipthousehold_input.configure(width=15)
        ...
        self.receiptdate_label.configure(text='Receipt Date', bg='#fff')
        self.receiptdate_input.configure(width=15)
        self.newreceipthousehold_label.configure(text='New Household name', bg='#fff')
        self.newreceipthousehold_input.configure(width=15)
        self.update_button.configure(text='UPDATE', padx=85)
        # assign event
        self.update_button.configure(command=self.update_receipt)

    def update_receipt(self):
        household = self.receipthousehold_input.get()
        receipt_number = self.receiptnumber_input.get()
        receipt_date = self.receiptdate_input.get()
        new_household = self.newreceipthousehold_input.get()
        receipt = Receipt(household, receipt_number, receipt_date).show()
        if receipt in self.Receipt:
            for i, item in enumerate(self.Receipt):
                if item == receipt:
                    self.Receipt[i] = Receipt(new_household, receipt_number, receipt_date).show()
            messagebox.showinfo(title="SUCCESS!!!", message="receipt updated successfully!")
        else:
            messagebox.showerror(title="ERROR!!!", message="update create failed!")
        self.receipthousehold_input.delete(0, 'end')
        self.receiptnumber_input.delete(0, 'end')
        self.receiptdate_input.delete(0, 'end')
        self.newreceipthousehold_input.delete(0, 'end')
